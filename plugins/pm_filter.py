        ...
        btn = [
            [
                InlineKeyboardButton(
                    ...
                    url=await get_shortlink(f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}", u_id=editable.reply_to_message.from_user.id)
                ),     
            ]
            for file in files
        ]
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}", 
                    url=await get_shortlink(f"https://telegram.me/{temp.U_NAME}?start=pre_{file.file_id}", u_id=editable.reply_to_message.from_user.id)
                ),
            ]
            for file in files
        ]
